...
// Get all participants (for admin or testing purposes only)
router.get("/", async (req, res) => {
  try {
    ...
    console.error(err);
    res.status(500).send({ message: "Error fetching participants" });
  }
});
...
router.post("/signup", async (req, res) => {
  try {
    ...
    res.status(201).send({ message: "participant registered successfully", id: result.insertedId,
      success: true
     });
  } catch (err) {
    console.error(err);
    res.status(500).send({ message: "Error registering participant" });
  }
});
